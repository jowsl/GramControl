...
test:
	g++ -fPIC -std=c++17 -I src -I includes -DPROJECT_ROOT_DIR='"$(PWD)"' tests/unit/main.cpp tests/unit/test_login_reg.cpp tests/unit/test_persistencia.cpp tests/unit/test_PrecoBase.cpp src/GramControl.cpp src/PrecoBase.cpp src/PrecoBaseController.cpp src/PrecoBaseDAO.cpp $(QT) -o bin/testes && rm -f gramcontrol.db && ./bin/testes

clean:
	rm -f bin/gramcontrol bin/testes gramcontrol.db src/moc_LoginWindow.cpp src/moc_AdminWindow.cpp src/moc_AtualizarPrecoDialog.cpp
